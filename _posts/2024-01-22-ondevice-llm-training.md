...
- Observed that training on mobilr CPUs is much faster than on mobile GPUs and identified two possible bottlenecks related to this observation: (i) computation and (ii) memory bottlenecks. To solve the computation bottleneck, we optimize the OpenCL backend’s kernels, showing 2x improvements (40-70 GFLOPs) over CPUs (15-30 GFLOPs) on the Snapdragon 8 series processors. 
...
- The authors find that for backpropagation, the ratios of GPU execution time to CPU execution time using both standard MNN kernels and the authors' optimized kernels are positive. This indicates that GPU training time is at least an order of magnitude slower than CPU training time. Interestingly, this ratio decreses as network size increases for the four model architectures the authors tested. 
